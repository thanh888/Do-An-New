...
@section('content')
    ...
    <div class="mt-4 mb-4 p-3 bg-white border shadow-sm lh-sm">
        ...
        <form class="row" method="post" action="{{ route('Role.update', ['id'=>$role->id]) }}" enctype="multipart/form-data">
            @csrf
            <div class="col-md-12">
                    <div class="form-group">
                    <label >Tên vai tro</label>
                    <input type="text" class="form-control" name="name"  placeholder="Nhap ten vai tro" value="{{ $role->name }}">
                    </div>
                    <div class="form-group">
                    <label >Mo ma vai tro</label>
                        ...
                        <div class="row">
                            @foreach ( $itemParent->permissionChildren as $itemChildren )
                                ...
                                <div class="card-body text-primary col-md-3">
                                    <label >
                                        <input type="checkbox" name="permission_id[]" class="checkbox_children" {{ $permissionchecked->contains('id', $itemChildren->id) ? 'checked': '' }} value="{{ $itemChildren->id }} ">
                                        {{ $itemChildren->name }} 
                                    </label>    
                                </div>
                            @endforeach
